...
use craft\commerce\elements\Order as CommerceOrderElement;
 ...
 * @since 2.0
 */
class CommerceOrder extends Element implements HasStoreInterface
{
    ...
    //         [['shippingAddress'], 'validateOrganizationTaxIdAsVatId', 'when' => fn(\propelcog\craftorderimport\elements\CommerceOrder $order) => $order->getStore()->getValidateOrganizationTaxIdAsVatId() && !$order->getStore()->getUseBillingAddressForTax()],
    //         [['billingAddress'], 'validateOrganizationTaxIdAsVatId', 'when' => fn(\propelcog\craftorderimport\elements\CommerceOrder $order) => $order->getStore()->getValidateOrganizationTaxIdAsVatId() && $order->getStore()->getUseBillingAddressForTax()],
    ...
    public function recalculate(): void
    {
        ...
        if ($this->getRecalculationMode() == self::RECALCULATION_MODE_ALL) {
            ...
            // if (OrderHelper::mergeDuplicateLineItems((CommerceOrderElement) $this)) {
            //     $recalculateOrder = true;
            // }
    ...
    private function _saveLineItems(): void
    {
        ...
        foreach ($this->getLineItems() as $lineItem) {
            ...
            $order = Plugin::getInstance()->getOrders()->getOrderById($lineItem->orderId);
            $lineItem->setOrder($order); // just in case.
